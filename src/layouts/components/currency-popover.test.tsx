...
describe('CurrencyPopover', () => {
     ...
     it('renders the Currencies button', async () => {
          ...
          const { getByLabelText } = renderWithProviders(<CurrencyPopover data={currencies} />, {
               theme: cssVarsTheme,
          });
     ...
     it('renders a flag image for the selected currency', async () => {
          const { CurrencyPopover } = await import('./currency-popover');
          const { container } = renderWithProviders(<CurrencyPopover data={currencies} />, { theme: cssVarsTheme });
     ...
     it('renders with empty data array', async () => {
          const { CurrencyPopover } = await import('./currency-popover');
          const { getByLabelText } = renderWithProviders(<CurrencyPopover data={emptyData} />, { theme: cssVarsTheme });
