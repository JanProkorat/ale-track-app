...
import { UpdateUserDto } from '../../../api/Client';
...
export function UserDetailView({
     ...
     const updateUser = useCallback(
          async (userId: string, userToUpdate: UpdateUserDto) => {
               ...
               let hasError = false;
               await executeApiCall(() => client.updateUserEndpoint(userId, userToUpdate), undefined, {
                    onError: () => {
                         hasError = true;
                    },
               });
     ...
     return (
          <>
               {user === undefined && <Typography sx={{ mt: 2, ml: 2 }}>{t('users.noDetailToDisplay')}</Typography>}
