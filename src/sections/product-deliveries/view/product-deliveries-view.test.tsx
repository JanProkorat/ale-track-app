     ...
     ProductDeliveryDetailView: ({
          ...
          <div data-testid="product-delivery-detail-view">
               ...
               <button
                    ...
                    onClick={() => onDeliveryChange({ ...delivery, note: 'changed' })}
               >
                    change
               </button>
          </div>
...
vi.mock('../detail-view/create-product-delivery-view', () => ({
     CreateProductDeliveryView: ({ onClose, onSave }: { onClose: () => void; onSave: (id: string) => void }) => (
...
describe('ProductDeliveriesView', () => {
     ...
     it('should render the new delivery button', async () => {
          ...
          await waitFor(() => {
               expect(screen.getByRole('button', { name: /productDeliveries\.new/ })).toBeInTheDocument();
          });
     });
     ...
     it('should render the product delivery detail view', async () => {
          ...
          await waitFor(() => {
               ...
               expect(screen.getByTestId('product-delivery-detail-view')).toHaveTextContent('has-delivery');
          });
     });
     ...
     it('should delete delivery and select the next one', async () => {
          ...
          await waitFor(() => {
               expect(mockShowSnackbar).toHaveBeenCalledWith('productDeliveries.deliveryDeleted', 'success');
               expect(screen.getByTestId('selected-id')).toHaveTextContent('del-2');
          });
     });
