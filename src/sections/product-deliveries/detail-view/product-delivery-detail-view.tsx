...
export function ProductDeliveryDetailView({ delivery, onDeliveryChange }: Readonly<ProductDeliveryDetailVProps>) {
     ...
     const fetchMultiselectData = useCallback(async () => {
          ...
          if (statesResult)
               setProductDeliveryStates(statesResult.map((state) => state as unknown as ProductDeliveryState));
     ...
     useEffect(() => {
          if (delivery) {
               const numericState =
                    ProductDeliveryState[delivery.state! as unknown as keyof typeof ProductDeliveryState];
     ...
     return (
          ...
          >
               {!detailLoading && delivery === undefined && (
                    <Typography sx={{ mt: 2, ml: 2 }}>{t('productDeliveries.noDetailToDisplay')}</Typography>
               )}
               {detailLoading && delivery === undefined && (
                    <LinearProgress
                         sx={{
