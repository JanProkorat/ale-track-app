...
export function CreateProductDeliveryView({ width, onClose, onSave }: Readonly<CreateProductDeliveryProps>) {
     ...
     const [delivery, setDelivery] = useState<CreateProductsDeliveryDto>(
          new CreateProductsDeliveryDto({
               deliveryDate: dayjs().toDate(),
               stops: [
                    new IdentifiedProductDeliveryStopDto({
                         ...
                         <DndContext
                              ...
                              onDragEnd={({ active, over }) => {
                                   if (active.id !== over?.id) {
                                        const oldIndex = (
                                             delivery.stops! as IdentifiedProductDeliveryStopDto[]
                                        ).findIndex((s) => s.id === active.id);
                                        const newIndex = (
                                             delivery.stops! as IdentifiedProductDeliveryStopDto[]
                                        ).findIndex((s) => s.id === over?.id);
