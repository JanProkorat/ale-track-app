...
const mockT = (key: string, params?: Record<string, string>) => (params ? `${key}:${JSON.stringify(params)}` : key);
...
vi.mock('../../notes/view/notes-view', () => ({
     NotesView: ({ parentId }: { parentId: string }) => <div data-testid="notes-view">{`Notes for ${parentId}`}</div>,
}));
