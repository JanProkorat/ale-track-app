...
export function ClientRemindersView({ clientId }: Readonly<ClientRemindersProps>) {
     ...
     const fetchReminders = useCallback(async () => {
          try {
               return await clientApi.fetchRemindersForClient(clientId, {}).then((data) =>
                    data.map(
                         (r) =>
                              new ReminderListItemDto({
                                   ...r,
     ...
     const resolveReminder = async (id: string, isResolved: boolean) => {
          ...
          const updatedReminders: ReminderListItemDto[] = [];

          originalReminders.forEach((reminder) => {
               if (reminder.id === id) reminder.isResolved = !isResolved;
          ...
          try {
               await clientApi.setClientReminderResolvedDateEndpoint(
                    id,
                    new SetClientReminderResolvedDateRequest({
     ...
     const reminderHeaderLabel = (reminder: ReminderListItemDto) => {
     ...
     return (
          <CollapsibleForm title={t('reminders.title')} headerChildren={newButton(filteredReminders.length > 0)}>
                                                                 ...
                                                                 slotProps={{
                                                                      primary: {
                                                                           sx: {
                                                                                ...
                                                                                textDecoration: reminder.isResolved
                                                                                     ? 'line-through'
                                                                                     : 'none',
                                                                           },
                                                                      },
                              ...
                              {selectedReminder !== null && (
                                   <Box>
                                        <SectionHeader text={selectedReminder.name!} headerVariant="h6" bold={false}>
                                             <Box sx={{ display: 'flex', alignItems: 'center' }}>
                                                  <IconButton
                                                       onClick={() => setSelectedReminderId(selectedReminder.id)}
                                                  >
               ...
               <Drawer anchor="right" open={selectedReminderId !== undefined} onClose={closeDrawer}>
