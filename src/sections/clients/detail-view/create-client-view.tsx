...
export function CreateClientView({ region, onClose }: Readonly<CreateClientViewProps>) {
    ...
    const handleChangeContacts = (contacts: {type: ContactType, description: string | undefined, value: string}[]) => {
        ...
        setClient(prev => new CreateClientDto({
            ...prev,
            contacts: contacts.map((contact) => new CreateClientContactDto({
                value: contact.value,
                type: contact.type,
            ...
            <Box sx={{display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', gap: 2}}>
                ...
                <FormControl fullWidth error={!!errors.name} sx={{mt: 1}}>
                    ...
                    <OutlinedInput
                        id="client-name"
                        value={client.name ?? ''}
                        onChange={event => setClient(prev => new CreateClientDto({
                            ...prev,
                ...
                <RegionSelect selectedRegion={client.region} errors={errors} onSelect={handleRegionSelect} maxWidth={30}/>
            </Box>
            ...
            <CollapsibleForm title={t('address.address')}>
                <Box sx={{ml: 1, mr: 1, mb: 2}}>
                    ...
                    <AddressForm
                        ...
                        errors={errors}
                        onChange={(newAddress) => setClient(prev => new CreateClientDto({
                            ...prev,
