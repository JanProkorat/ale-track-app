...
const slot2 = createSlot(
    tomorrow.add(1, 'day').hour(9).toDate(),
    tomorrow.add(1, 'day').hour(17).toDate()
);
...
describe('DriverAvailabilityEditor', () => {
    ...
    it('should display multiple availability slots', () => {
        render(
            <DriverAvailabilityEditor availableDates={[slot1, slot2]} onChange={mockOnChange} />
        );

        expect(screen.getByText(tomorrow.format('DD. MM. YYYY'))).toBeInTheDocument();
        expect(
            screen.getByText(tomorrow.add(1, 'day').format('DD. MM. YYYY'))
        ).toBeInTheDocument();
    });
    ...
    it('should highlight overlapping slots', () => {
        const overlappingSlot1 = createSlot(
            tomorrow.hour(8).toDate(),
            tomorrow.hour(14).toDate()
        );
        const overlappingSlot2 = createSlot(
            tomorrow.hour(12).toDate(),
            tomorrow.hour(18).toDate()
        );
