...
export function DriversAvailabilityCalendar({ drivers }: Readonly<{ drivers: DriversProps[] }>) {
     ...
     const getAvailableDrivers = (date: dayjs.Dayjs) =>
          drivers.filter((driver) =>
               driver.availableDates?.some((interval) => {
                    ...
                    return (
                         ...
                         date.isSame(until, 'day') ||
                         (date.isAfter(from, 'day') && date.isBefore(until, 'day'))
                    );
               })
          );

     const handleDriverClick = useCallback(
          (event: React.MouseEvent<HTMLElement>, driver: DriversProps, date: dayjs.Dayjs) => {
               const intervals = driver.availableDates.filter(
                    (interval) =>
                         interval.from &&
                         interval.until &&
                         dayjs(date).isBetween(
                              dayjs(interval.from).startOf('day'),
                              dayjs(interval.until).endOf('day'),
                              null,
                              '[]'
                         )
               );
