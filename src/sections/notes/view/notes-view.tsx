...
export function NotesView({ parentId, parentType }: Readonly<NotesViewProps>) {
     ...
     const fetchClientNotes = useCallback(
          async () => await executeApiCallWithDefault(() => client.getClientNotesEndpoint(parentId), []),
          [parentId, executeApiCallWithDefault, client]
     );

     useEffect(() => {
          if (parentType === SectionType.Client) fetchClientNotes().then(setNotes);
     }, [fetchClientNotes, parentId, parentType]);

     const closeDrawer = (shouldRefresh: boolean) => {
          if (shouldRefresh) fetchClientNotes().then(setNotes);
     ...
     const noDataBox = (
          <Box sx={{ py: 5, textAlign: 'center', width: '100%' }}>
               ...
               <Box sx={{ mt: 2, display: 'flex', justifyContent: 'center' }}>{newButton(notes.length === 0)}</Box>
          </Box>
     );
