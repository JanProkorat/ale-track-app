...
vi.mock('../detail-view/outgoing-shipment-detail-view', () => ({
    OutgoingShipmentDetailView: ({
        ...
        <div data-testid="outgoing-shipment-detail-view">
            ...
            <button
                data-testid="mock-change-shipment"
                onClick={() => onChange({ ...shipment, name: 'changed' })}
            >
                change
            </button>
        </div>
...
describe('OutgoingShipmentsView', () => {
    ...
    it('renders the new shipment button', async () => {
        ...
        await waitFor(() => {
            expect(
                screen.getByRole('button', { name: /outgoingShipments\.new/ })
            ).toBeInTheDocument();
        });
    });
    ...
    it('renders detail view when a shipment is selected', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByTestId('outgoing-shipment-detail-view')).toHaveTextContent(
                'has-shipment'
            );
        });
    });
    ...
    it('shows empty state message when no shipments', async () => {
        ...
        await waitFor(() => {
            expect(
                screen.getByText('outgoingShipments.noShipmentsToDisplay')
            ).toBeInTheDocument();
        });
    });
