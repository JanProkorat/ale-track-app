...
export function OrdersSelect({
  ...
  return (
    <FormControl disabled={disabled} fullWidth sx={{ mt: 1 }} error={shouldValidate && selectedOrders.length === 0}>
      ...
      <Select
        ...
        onChange={(e) => {
          ...
          const ordersWithWeights: [string, number][] = selectedOrderIds.map((orderId) => {
            ...
            const totalWeight = order?.items?.reduce((sum, item) => sum + (item.weight || 0) * (item.quantity || 0), 0) || 0;
