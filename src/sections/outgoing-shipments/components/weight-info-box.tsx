...
export function WeightInfoBox({ currentWeight, maxWeight }: Readonly<WeightInfoBoxProps>) {
     ...
     return (
          <FormControl
               disabled
               error={currentWeight !== undefined && maxWeight !== undefined && currentWeight > maxWeight}
               fullWidth
               sx={{ mt: 1 }}
          >
               <InputLabel htmlFor="shipment-weight">{t('outgoingShipments.weight') + ' (Kg)'}</InputLabel>
               <OutlinedInput id="shipment-weight" value={value} label={t('outgoingShipments.weight') + ' (Kg)'} />
          </FormControl>
     );
}
