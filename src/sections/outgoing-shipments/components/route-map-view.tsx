...
export default function RouteMapView({ type, stops, isFullscreen, onFullScreenToggle }: RouteMapProps) {
     ...
     useEffect(() => {
          setAddressesToDisplay([
               companyAddress,
               ...stops.map((stop) => stop.selectedAddress).filter((address) => address !== undefined),
               companyAddress,
          ]);
     ...
     const createStartEndIcon = (label: string) =>
          L.divIcon({
     ...
     useEffect(() => {
          ...
          if (!mapRef.current) {
               mapRef.current = L.map(containerId).setView(
                    [addressesToDisplay[0].latitude ?? 0, addressesToDisplay[0].longitude ?? 0],
                    8
               );
          ...
          const isSameStartEnd =
               firstAddress.latitude === lastAddress.latitude && firstAddress.longitude === lastAddress.longitude;
          ...
          addressesToDisplay.forEach((address, index) => {
               ...
               if (index === 0 && isSameStartEnd) {
                    ...
                    marker = L.marker([address.latitude ?? 0, address.longitude ?? 0], {
          ...
          routingRef.current = (L as any).Routing.control({
               waypoints: addressesToDisplay.map((c) => L.latLng(c.latitude ?? 0, c.longitude ?? 0)),
