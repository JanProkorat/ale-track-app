  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Box sx={{ mt: 2, display: 'flex', gap: 2, flex: 1, minHeight: 0 }}>
            ...
            >
              {routeStops.map((stop) => (
                <SortableRouteStop
                  ...
                  selectedAddressType={
                    selectedAddresses[stop.orderId] || OutgoingShipmentStopAddressKind.Official
                  }
                  onToggle={() => toggleStop(stop.orderId)}
                  onAddressSelect={(addressType) => handleAddressSelect(stop.orderId, addressType)}
        ...
        >
          {routeStops.length > 0 ? (
            <RouteMapView type={type} stops={routeStops} isFullscreen={false} onFullScreenToggle={setMapFullScreenOpen} />
          ) : (
            <Card sx={{ p: 2 }}>
        ...
        <Dialog open={mapFullScreenOpen} onClose={() => setMapFullScreenOpen(false)} fullScreen>
          ...
          <DialogContent sx={{ p: 0, height: '100vh' }}>
            <RouteMapView type={type} stops={routeStops} isFullscreen onFullScreenToggle={setMapFullScreenOpen} />
          </DialogContent>
        </Dialog>
      </Box>
    </Box>
