...
import { OrderDto, OrderItemDto, ClientInfoDto, GroupedProductHistoryDto } from '../../../api/Client';

import type { OrderState, UpdateOrderDto } from '../../../api/Client';
...
vi.mock('./update-order-view', () => ({
     UpdateOrderView: ({ disabled, order }: { disabled: boolean; order: UpdateOrderDto }) => (
...
const mockOrderDetail = new OrderDto({
     ...
     orderItems: [new OrderItemDto({ productId: 'prod-1', quantity: 10 })],
});
...
describe('OrderDetailView', () => {
     ...
     it('should show no detail message when id is undefined', () => {
          render(<OrderDetailView id={undefined} {...defaultProps} />);
     ...
     it('should fetch order detail when id is provided', async () => {
          ...
          render(<OrderDetailView id="order-1" {...defaultProps} />);
     ...
     it('should disable UpdateOrderView for cancelled orders', async () => {
          ...
          render(<OrderDetailView id="order-2" {...defaultProps} />);
