...
import {
    ReminderType,
    type DayOfWeek,
    UpdateReminderDto, ReminderRecurrenceType
} from "../../../api/Client";
...
export function UpdateReminderView({reminderId, parentType, onClose}: Readonly<UpdateReminderViewProps>) {
    ...
    const fetchReminder = useCallback(async (id: string): Promise<void> => {
        try {
            ...
            const mappedRecurrence = mapEnumValue<ReminderRecurrenceType>(ReminderRecurrenceType, data.recurrenceType);

            setReminder(new UpdateReminderDto({
                ...data,
                type: mappedType!,
