...
export const CurrencyProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
     ...
     const formatPriceValue = useCallback(
          (value: number | undefined) => {
               ...
               const amount =
                    (value ?? 0) /
                    (selectedCurrency.currencyCode === czechRate.currencyCode ? 1 : (selectedCurrency.rate ?? 1));
     ...
     const formatPrice = useCallback(
          (value: number | undefined) => {
               ...
               const currency = selectedCurrency.currencyCode === czechRate.currencyCode ? 'Kč' : '€';
               return `${amount?.toFixed(2) ?? ''} ${amount !== undefined ? currency : ''}`;
          },
          [selectedCurrency, formatPriceValue]
     );

     const formatPriceToDefault = useCallback(
          (value: number | undefined) => {
