...
describe('TableToolbar', () => {
     it('renders filters when nothing is selected', () => {
          renderWithProviders(<TableToolbar numSelected={0} filters={[<input key="f1" placeholder="Filter 1" />]} />);
     ...
     it('renders multiple filters', () => {
          renderWithProviders(
               <TableToolbar
                    numSelected={0}
                    filters={[<input key="f1" placeholder="Filter A" />, <input key="f2" placeholder="Filter B" />]}
               />
          );
     ...
     it('hides filters when items are selected', () => {
          renderWithProviders(
               <TableToolbar numSelected={3} filters={[<input key="f1" placeholder="Hidden filter" />]} />
          );
          expect(screen.queryByPlaceholderText('Hidden filter')).not.toBeInTheDocument();
     });

     it('shows filters when numSelected is 0', () => {
          renderWithProviders(<TableToolbar numSelected={0} filters={[<span key="f1">Visible</span>]} />);
          expect(screen.getByText('Visible')).toBeInTheDocument();
          expect(screen.queryByText(/table\.selected/)).not.toBeInTheDocument();
     });

     it('renders with empty filters array', () => {
          const { container } = renderWithProviders(<TableToolbar numSelected={0} filters={[]} />);
