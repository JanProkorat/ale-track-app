...
import { CreateProductDeliveryItemDto, CreateProductDeliveryStopDto } from './Client';

import type { ICreateProductDeliveryStopDto, ICreateProductDeliveryItemDto } from './Client';
...
export class IdentifiedProductDeliveryStopDto extends CreateProductDeliveryStopDto {
     ...
     constructor(data?: ICreateProductDeliveryStopDto & { id?: string }) {
          ...
          if (data?.products) {
               this.products = data.products.map((p) => new IdentifiedProductDeliveryItemDto(p));
          }
     }
     ...
     override init(_data?: any) {
          ...
          if (Array.isArray(_data['products'])) {
               this.products = _data['products'].map((p: any) => IdentifiedProductDeliveryItemDto.fromJS(p));
          }
     }
