...
describe('handleApiCallWithDefault', () => {
     ...
     it('should return result on success', async () => {
          const result = await handleApiCallWithDefault(() => Promise.resolve([1, 2, 3]), []);

          expect(result).toEqual([1, 2, 3]);
     });

     it('should return default value on error', async () => {
          const result = await handleApiCallWithDefault(() => Promise.reject(new Error('fail')), [0]);

          expect(result).toEqual([0]);
     });

     it('should return empty array default on error', async () => {
          const result = await handleApiCallWithDefault(() => Promise.reject(new Error('fail')), [] as number[]);
